	...
	public override void OnInspectorGUI()
	{
		...
		if (scaling == UIRoot.Scaling.Flexible)
		{
			...
			EditorGUILayout.HelpBox(
				"Also known as the 'Pixel-Perfect' mode, this setting makes NGUI's virtual pixels match the screen. This means that your UI will look smaller on high resolution devices and bigger on lower resolution devices, but it will always be as crisp as it can be." +
		...
		else
		{
			...
			if (fitHeight)
			{
				...
				{
					EditorGUILayout.HelpBox(
						"Whatever you create within the content frame (blue outline) will always be visible, regardless of the screen's aspect ratio." +
						"\n\nThink of it as choosing the 'Fit' desktop background style.", MessageType.Info);
				}
				else
				{
					EditorGUILayout.HelpBox(
						"This setting will keep your UI look the same on all screen sizes relative to the height of the screen." +
			...
			{
				EditorGUILayout.HelpBox(
					"This setting will keep your UI look the same on all screen sizes relative to the width of the screen." +
			...
			{
				EditorGUILayout.HelpBox(
					"Your UI within the content frame (blue outline) will always fill the screen, and edges will be cropped based on the aspect ratio." +
